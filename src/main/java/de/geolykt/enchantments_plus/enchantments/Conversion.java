            ...
            if (player.isSneaking()) {
                if (player.getLevel() > 1) {
                    if (player.getHealth() < 20) {
                        ...
                        for (int i = 0; i < 3; i++) {
                            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Storage.plugin, () -> CompatibilityAdapter.display(Utilities.getCenter(player.getLocation()), Particle.HEART,
                                    10, .1f, .5f, .5f, .5f), ((i * 5) + 1));
                        }
                        return true;
                    }
                }
